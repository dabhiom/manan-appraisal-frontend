      ...
      <table id="employees">
        <thead>
          <tr>
            ...
            >
              Department{" "}
              <span className="sort">{renderSortIndicator("department")}</span>
            </th>
            {!showInvalid && (
              <>
                <th
                  className="num"
                  data-key="currentsalary"
                  onClick={() => onSort("currentsalary")}
                  style={{ cursor: "pointer" }}
                >
                  Current Salary{" "}
                  <span className="sort">
                    {renderSortIndicator("currentsalary")}
                  </span>
                </th>
                <th
                  data-key="grade"
                ...
                >
                  Grade{" "}
                  <span className="sort">{renderSortIndicator("grade")}</span>
                </th>
                <th
                  className="num"
                  data-key="increment"
                  onClick={() => onSort("increment")}
                  style={{ cursor: "pointer" }}
                >
                  Increment (%){" "}
                  <span className="sort">
                    {renderSortIndicator("increment")}
                  </span>
                </th>
                <th
                  className="num"
                  data-key="incrementedsalary"
                  onClick={() => onSort("incrementedsalary")}
                  style={{ cursor: "pointer" }}
                >
                  Incremented Salary{" "}
                  <span className="sort">
                    {renderSortIndicator("incrementedsalary")}
                  </span>
                </th>
              </>
            )}
            {showInvalid && (
              <>
                ...
                <th
                  className="num"
                  data-key="kpiscore"
                  onClick={() => onSort("kpiscore")}
                  style={{ cursor: "pointer" }}
                >
                  KPI{" "}
                  <span className="sort">
                    {renderSortIndicator("kpiscore")}
                  </span>
                </th>
                <th
                  className="num"
                ...
                <th
                  className="num"
                  data-key="behavioralrating"
                  onClick={() => onSort("behavioralrating")}
                  style={{ cursor: "pointer" }}
                >
                  Behavior{" "}
                  <span className="sort">
                    {renderSortIndicator("behavioralrating")}
                  </span>
                </th>
                <th
                  className="num"
                  data-key="managerrating"
                  onClick={() => onSort("managerrating")}
                  style={{ cursor: "pointer" }}
                >
                  Manager{" "}
                  <span className="sort">
                    {renderSortIndicator("managerrating")}
                  </span>
                </th>
              </>
            )}
        ...
        <tbody>
          {employees.map((emp, index) => (
            ...
            >
              <td>{highlight(emp.id ?? "", searchQuery)}</td>
              <td>{highlight(emp.name ?? "", searchQuery)}</td>
              <td>{highlight(emp.department ?? "", searchQuery)}</td>
              {!showInvalid && (
                <>
                  <td className="num">{formatMoney(emp.currentsalary)}</td>
                  <td>{emp.grade}</td>
                  <td className="num">
                    {emp.increment ? `${emp.increment}%` : "—"}
                  </td>
